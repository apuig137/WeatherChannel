...
let cargarCiudad = async (ciudadElegida) => {
    await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${ciudadElegida}&lang=sp&appid=7f5482804e804c168f596be3f1c6ed56`)
        .then( (resp) => resp.json() )
        .then( (data) => {
            if(data.cod === "404"){
    ...
    fetch(`https://restcountries.com/v3.1/alpha/${pais}`)
        ...
        .then( (data) => {
            ...
            document.querySelector("#pais").textContent = data[0].name.common
